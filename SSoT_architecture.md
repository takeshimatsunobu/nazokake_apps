...
## 8.4 Agent Security & Quality Assurance (エージェントの保護と品質担保)

エージェントの自律稼働に伴うシステムの自己破壊リスクを排除し、生成されるコードの品質を継続的に担保するため、以下のセキュリティおよびフェイルセーフ機構を**実装済み**の標準動作として厳格に運用する。

*   **Sandbox環境の物理的保護 (tools/ ディレクトリのRead-Onlyマウント)**
    エージェントが自身の思考プロセスやコアツール群を誤って変更・削除・破壊することを防ぐため、`infra/docker-compose.yml` において `agent-workspace` サービスの `tools/` ディレクトリは必ず `:ro` (Read-Only) オプションを付与してマウントする。これにより、プロンプトによるソフトな制限だけでなく、インフラレベルでの自己改変を完全にブロックする。

*   **決定論的テストの強制 (pyright_tool.py のワークフロー統合)**
    エージェントのコード自動修正における品質退行を防ぐため、自己修復ワークフロー (`tools/agent_graph.py`) に静的型チェック機能 (`typecheck_node`) を完全に統合している。エージェントがコードの修正（AST置換等）を行った後は、必ず pyright による型検査が自動実行され、エラーが検出された場合は修正サイクルへと強制的に差し戻される。これにより、型安全性が担保されないコードの適用を未然に防ぐ（Fail-Closedの徹底）。
