...
def _fetch_ai_scores_sync(db, doc_id: str, model_target: str) -> Optional[dict]:
    """指定doc_idのnazokake_itemsから、model_targetに対応するAIの11軸スコアを取得する。"""
...
async def analyze_axis_divergence(db, limit: int = 100) -> dict:
    ...
    for fb in feedbacks:
        ...
        ai_scores = await asyncio.to_thread(
            _fetch_ai_scores_sync, db, doc_id, model_target
        )
