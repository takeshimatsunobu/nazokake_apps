...
def _fetch_fewshot_entry_sync(db, doc_id: str):
    """指定doc_idのnazokake_itemsから、Few-shotプール互換のCoT辞書を組み立てる(取得できる範囲のみ)。
...
async def build_high_rated_fewshot_entries(db, limit: int = 200) -> list:
    ...
    for doc_id in doc_ids:
        entry = await asyncio.to_thread(_fetch_fewshot_entry_sync, db, doc_id)
        if entry:
