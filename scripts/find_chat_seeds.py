import firebase_admin
from firebase_admin import credentials, firestore
from pathlib import Path

def find_injected_seeds():
    key_path = Path.cwd() / "backend" / "serviceAccountKey.json"
    if not firebase_admin._apps:
        if key_path.exists():
            cred = credentials.Certificate(str(key_path))
            firebase_admin.initialize_app(cred)
        else:
            firebase_admin.initialize_app()
            
    db = firestore.client()
    
    print("🔍 注入した3作品（Takeshi_Gemini_Brainstorm）を捜索中...")
    try:
        # 著者名で完全一致検索
        docs = db.collection("nazokake_items").where("author", "==", "Takeshi_Gemini_Brainstorm").stream()
        
        found = False
        for doc in docs:
            found = True
            data = doc.to_dict()
            title = data.get("A_TITLE", "不明")
            status = data.get("status", "不明")
            created_at = data.get("created_at")
            created_at_type = type(created_at).__name__
            has_scores = "あり" if len(data.get("scores", {})) > 0 else "なし"
            
            print(f"📌 お題: {title:<10} | Status: {status} | AI評価: {has_scores} | 作成日時型: {created_at_type}")
            
        if not found:
            print("⚠️ 注入したデータがデータベース上に見つかりません。")
            
    except Exception as e:
        print(f"🚨 エラー発生: {e}")

if __name__ == "__main__":
    find_injected_seeds()
