...
// app.js（Controller）から uiRenderFeed(feedItems) として呼び出される。
...
function renderFeedItem(item) {
    ...
    // A_TITLE は現在のバックエンド契約(FeedItem)には存在しない旧Firestore時代の
    // フォールバックフィールド。実装が生きているかを示すため型は明示的にanyで逃がす。
    const docId = item.doc_id || item.id; const odai = item.odai || /** @type {any} */ (item).A_TITLE || "不明";
    ...
    if (isEvaluated) {
        ...
        <div class="bg-[#FAF8F5] rounded-xl p-4 border border-[#C5B358]/50 shadow-inner mt-4">
            ...
            <div class="flex gap-2 mb-4">${[1,2,3,4,5].map(i => `<button data-action="setFeedRating" data-doc-id="${docId}" data-score="${i}" id="feed-star-${docId}-${i}" class="flex-1 py-2 border border-[#C5B358]/50 bg-white text-gray-400 rounded-md font-bold text-sm shadow-sm transition hover:scale-105">⭐${i}</button>`).join('')}</div>
            ...
            <button data-action="submitUserEvaluation" data-doc-id="${docId}" class="w-full bg-[#2C3539] text-[#C5B358] py-3 rounded-lg text-sm font-bold shadow-md transition duration-200 hover:bg-gray-800">📤 確定して次へ</button>
        </div>`;
    }
 ...
 * @param {import('../../api').components['schemas']['FeedItem'][]} feedItems
 */
export function uiRenderFeed(feedItems) {
    if (!feedItems) return;
    feedItems.forEach(item => renderFeedItem(item));
}

// 道場破りカードの星評価の選択状態を反映する（data-action="setFeedRating" から呼ばれる）
export function setFeedRating(docId, score) {
    /** @type {HTMLInputElement} */ (document.getElementById(`feed-score-${docId}`)).value = score;
    for(let i=1; i<=5; i++) {
...
export function uiFeedClear() {
    const c = document.getElementById('feed-container');
    if (c) c.innerHTML = '';
}

// 追加読み込み中: sentinel にローディングのドットアニメを表示する。
export function uiFeedShowLoading() {
...
export function uiFeedShowRetry(onRetry) {
    const s = document.getElementById('feed-sentinel');
    if (!s) return;
...
export function uiFeedGetEvaluationInput(docId) {
    const val = (id) => /** @type {HTMLInputElement | null} */ (document.getElementById(id))?.value;
    return {
...
export function uiFeedRemoveCard(docId) {
