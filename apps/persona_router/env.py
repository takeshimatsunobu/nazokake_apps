"""
apps/persona_router/env.py
=============================
このアプリ専用の .env (apps/persona_router/.env) を読み込むヘルパー。
...
override=True で読み込む(apps/persona_router はその探索経路の祖先ディレクトリ
...
【呼び出し順序が重要】
load_persona_router_env() は、`router`(→`llm`→nazokake_core.env_config)が
...
def load_persona_router_env() -> None:
